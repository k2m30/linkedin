...
configure do
  ...
  set :api, "77t2v9vcniqt0s"
  set :secret, "MvFQtxReXq2eS7pB"
end
...
get "/auth" do
  client = LinkedIn::Client.new(settings.api, settings.secret)
  request_token = client.request_token({}, :oauth_callback => "http://#{request.host}:#{request.port}/auth/callback", :scope => "r_basicprofile r_emailaddress")
